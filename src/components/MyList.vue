  ...
  <div id="myList" class="myList">
    <ul>
      ...
      <li v-for="item in list.slice(0, 7)" :key="item.id">
        ...
        >
          ...
          <p>
            ...
            <span v-show="item.price != item.selling_price" class="del"
              >{{ item.price }}元</span
            >
          </p>
        </router-link>
      ...
      <li v-show="isMore && list.length >= 1" id="more">
        <!-- router 路由跳转 /goods/categforyId=1-->
        <router-link
...
export default {
  ...
  methods: {
    deleteCollect(product_id) {
      this.$axios
        .delete("/goods/collects/del/", {
          data: {
            user: this.$store.getters.getUser, //打印出来看一下
            product_id: product_id,
          },
        })
        .then((res) => {
          //
          console.log("@@删除收藏的响应：", res.data);
